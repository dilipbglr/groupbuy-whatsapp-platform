...
router.post('/deals', async (req, res) => {
    try {
        ...
        const { data, error } = await supabaseClient_1.supabase.from('deals').insert([deal]).select();
        if (error)
            throw error;
...
router.post('/deals/:id/join', async (req, res) => {
    ...
    try {
        ...
        const { data: existing, error: dupError } = await supabaseClient_1.supabase
            .from('participants')
            .select('id')
            .eq('deal_id', dealId)
        ...
        const { error: insertError } = await supabaseClient_1.supabase.from('participants').insert([{
                deal_id: dealId,
                phone_number,
                quantity,
                payment_status: 'paid',
        ...
        if (updateError)
            throw updateError;
        const messageText = updates.status === 'success'
        ...
        await (0, sendWhatsApp_1.sendWhatsAppMessage)(phone_number, messageText);
...
router.get('/deals/:id/status', async (req, res) => {
    const dealId = req.params.id;
    try {
        const { data: deal, error } = await supabaseClient_1.supabase
            .from('deals')
        ...
        const progressPercent = Math.round((deal.current_participants / deal.max_participants) * 100);
        return res.json({
            success: true,
            deal_id: deal.id,
